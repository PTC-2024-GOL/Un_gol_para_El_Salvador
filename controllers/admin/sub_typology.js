...
const openUpdate = async (id, nombre) => {
    try {
        ...
        if (DATA.status) {
            // Se muestra la caja de diálogo con su título.
            SAVE_MODAL.show();
            MODAL_TITLE.textContent = `Actualizar subtipología: ${nombre}`;
            // Se prepara el formulario.
            SAVE_FORM.reset();
...
const openDelete = async (id, nombre) => {
    ...
    const RESPONSE = await confirmAction(`¿Desea eliminar la subtipología: ${nombre} de forma permanente?`);
    try {
    ...
    subTipologiaPagina.forEach(row => {
        const tablaHtml = `
            <tr>
                ...
                <td>${row.NOMBRE}</td>
                <td>
                    <button type="button" class="btn transparente" onclick="openUpdate(${row.ID}, '${row.TIPOLOGIA}/${row.NOMBRE}')">
                        <img src="../../../resources/img/svg/icons_forms/pen 1.svg" width="18" height="18">
                    </button>
                    <button type="button" class="btn transparente" onclick="openDelete(${row.ID}, '${row.TIPOLOGIA}/${row.NOMBRE}')">
