...
const API = 'services/admin/tareas.php';
...
const openCreate = () => {
    ...
    SAVE_MODAL.show();
    MODAL_TITLE.textContent = 'Agregar una tarea';
...
const openUpdate = async (id, nombre) => {
    try {
        ...
        if (DATA.status) {
            // Se muestra la caja de diálogo con su título.
            SAVE_MODAL.show();
            MODAL_TITLE.textContent = `Actualizar la tarea: ${nombre}`;
            // Se prepara el formulario.
            SAVE_FORM.reset();
            // Se inicializan los campos con los datos.
            const ROW = DATA.dataset;
            ID_TAREA.value = ROW.ID;
            NOMBRE_TAREA.value = ROW.NOMBRE;
        } else {
...
const openDelete = async (id, nombre) => {
    ...
    const RESPONSE = await confirmAction(`¿Desea eliminar la tarea: ${nombre}?`);
    try {
...
window.onload = async function () {
    ...
    const contentHtml = await loadComponent('../components/task.html');
    ...
    SAVE_FORM.addEventListener('submit', async (event) => {
        ...
        (ID_TAREA.value) ? action = 'updateRow' : action = 'createRow';
